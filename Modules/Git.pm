...
sub fake_git_output
{
	...
	if ($message->message =~ /^git\s+(\w+?|--help)(\s|$)/) {
		...
		} elsif ($command eq 'pull' || $command eq 'push' || $command eq 'fetch' || $command eq 'checkout' || $command eq 'branch' || $command eq 'add' || $command eq 'mv' || $command eq 'reset' || $command eq 'rm' || $command eq 'diff' || $command eq 'rebase' || $command eq 'merge' || $command eq 'bisect' || $command eq 'commit') {
